...
Without doing any reordering We cannot identify any clusters or outliers. The heat map looks randomized.
```{r heatmap}
...
Based on the euclidean distance heat map, net wage tends to higher values from Dubai up towards Tokyo while the number of hours worked decrease. This is the opposite of cities like Delhi,Bangkok and Seoul. Interestingly food costs are generally low in the cities with higher working hours. Caracas is an outlier because food costs are high while net wage and the number of hours worked remains low.
...
From visual comparison of the heat map produced by HC solver and TSP, we prefer the latter because there appears to be a separation along the anti diagonal such that cities that have similarly high prices are placed along this diagonal. In the heat map by the TSP solver this distinction is not quite clear.
...
Cairo is the most distinct outlier with respect to the price of bread per kg and net wage being very low while average number of work hours are very high. Prague, Johannesburg and Panama form a cluster the prices are comparatively small and similar across the products. Luxembourg, Oslo, Zurich, and Geneva form a cluster, they have similar indicators with vacation days proportional to net wage and high price of goods and services. Helsinki, Frankfurt, Vienna, Munich, Lyon, Paris and Copenhagen also form a cluster. Miami, Los Angeles, Chicago, Montreal and Toronto also form a cluster.   

### 1.7

Among Heat maps, parallel coordinates and radar charts, heat maps are relatively easier to interpret in terms of time and accuracy. Radar charts are hard to interpret when the number of variables to compare are high. We found parallel coordinates to be generally messy to work with.
...
The points in the scatter plot for both income levels are very close to each other and it is difficult to interpret the results for both <=50K and >50K income levels.
The trellis plot shows both income levels in separate panels and so, it is quite easy to analyse the data. The people with age range of 0-25 who earns <=50K are working for long hours than people earning >50K.
...
The trellis plot exhibits both income level for people with various marital status. The density curve for widower is normally distributed for both income levels. The people who were never married earning <=50K lies within range of 17-40 age group.
...
In 3D-scatter plot, the overlapping of data points around the same values makes it difficult to analyse the relationship between given three variables.
...
In raster type 2D-density plot, there are 6 panels for each specific age group. Every panels of age group from [29,35] to [46,54] exhibits almost same outputs for capital loss ranging 1000-2000 except age group of [17,29] and [54,90]. There seems higher density in panel 1 with capital loss ranging from 1000-2000 and in panel 6, there is no such higher density so, it implies that people with age 54 to 90 has no such high capital loss when compared to other age groups.  

```{r 2D Density plot}
plot5 <- ggplot(df1, aes(x=edu_num,y=cap_loss))+
         stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE)+
         scale_fill_distiller(palette= "Spectral", direction=1) +scale_x_continuous(expand = c(0, 0)) +
         scale_y_continuous(expand = c(0, 0)) +labs(title="2D raster type density plot")+
         theme(legend.position='none')+facet_wrap(~cut_number(df1$age,6))
plot5
```

### 2.4 Trellis plots

The age is divided into 4 groups and plotted in four panels. The patterns in 2 panels ([17,28] & [28,37])of trellis plot looks almost similar and the same exists for other two panels.

```{r (a) using cut number}
plot6a<-ggplot(df, aes(x=edu_num,y=cap_loss,color=cap_loss))+geom_point()+labs(title="Trellis plot")+
  facet_wrap(~cut_number(df$age,4))
plot6a
```

Here, 10% overlap is created. In this specific problem, the trellis plot with shingles has no such big advantage. The plots with and without overlapping almost looks the same except some data points in plot with shingles. Ideally,  shingles should create an overlap to avoid information loss when plotting split data.

```{r (b) using shingles}
Agerange<-lattice::equal.count(df$age, number=4, overlap=0.1) #overlap is 10% 

M<-matrix(unlist(levels(Agerange)), ncol=2, byrow = T)
