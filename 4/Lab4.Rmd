...
# number of plots equal to number of columns
nplot <- nrow(p_ord_HC)

p_ord_HC %>%
  mutate_each(funs(rescale), -name) -> p_ord_HC_r

for (i in 1:nPlot){
  plots[[i]] <- htmltools::tags$div(
    plot_ly(type = 'scatterpolar', 
            r=as.numeric(p_ord_HC_r[i,-1]),
            theta= colnames(p_ord_HC_r)[-1], 
            fill="toself")%>%
      layout(title=p_ord_HC_r$name[i]), style="width: 25%;")
}
...
Cairo and Delhi are outliers with respect to the price of bread per kg. Prague, Johanessburg and Panama form a cluster the prices are comparatively small and similar across the products.. The most distinct outlier would be Caracas. The prices are higher due to the size of the radar plot.

### 1.7

Among Heatmaps, paralled coordinates and radar charts, heatmaps are relatively easier to interprate in terms of time and accuracy. Radar cahrts are hard to interprate when the number of variables to compare are high. We found parallel coordiantes to be generally messy to work with.
